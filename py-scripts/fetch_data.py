...
def get_report(url, name, target_date):
    ...
    if date_element:
        ...
        try:
            ...
            return (
                pd.DataFrame(),
                None,
            )  # Return an empty DataFrame if the date format is unexpected

        if report_date != target_date:
            return (
                pd.DataFrame(),
                report_date,
            )  # Return an empty DataFrame with the found date if the date does not match the target date
    else:
    ...
    if table:
        for row in table.find_all("tr")[
            2:-2
        ]:  # Skip the first two rows (headers) and last 2 rows (footer)
            ...
            if len(cols) == 4:  # Ensure it's a data row
                ...
                anglers = int(
                    re.search(r"\d+", anglers_text).group()
...
def get_all_reports(urls, target_date):
    ...
    if reports:
        ...
        return (
            pd.DataFrame(),
            dates,
        )  # Return an empty DataFrame if no reports are found
